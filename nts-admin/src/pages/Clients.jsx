...
/* ─── Stat pill ─────────────────────────────────── */
function StatPill({ label, value, color = T.navy }) {
  return (
    <Box sx={{
    ...
    <Box sx={{ pb: 4 }}>
      ...
      <Box sx={{
        ...
        <Box>
          <Typography sx={{
            ...
            fontSize: '1.75rem', fontWeight: 400, color: T.navy, lineHeight: 1.1,
          }}>
            Client Registry
          </Typography>
          <Typography sx={{ color: T.slate, fontSize: '0.82rem', mt: 0.5 }}>
            {clients.length} client{clients.length !== 1 ? 's' : ''} across {firms.length} firm{firms.length !== 1 ? 's' : ''}
          </Typography>
        </Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
          ...
          <Box sx={{ display: { xs: 'none', md: 'flex' }, gap: 1.5 }}>
            <StatPill label="Total" value={clients.length} color={T.navy} />
            <StatPill label="With Firm" value={withFirm} color="#1565c0" />
            <StatPill label="With Email" value={withEmail} color={T.green} />
          </Box>
          ...
          <Button
            ...
            sx={{
              ...
              background: T.navy, fontWeight: 700, fontSize: '0.83rem',
              textTransform: 'none',
              boxShadow: '0 4px 14px rgba(13,27,42,0.25)',
              '&:hover': { background: T.navy2, boxShadow: '0 6px 20px rgba(13,27,42,0.35)', transform: 'translateY(-1px)' },
              transition: 'all 0.2s',
            }}
          >
            Add Client
      ...
      <Box sx={{
            ...
            InputProps={{
              startAdornment: (
                <InputAdornment position="start">
                  <SearchRounded sx={{ fontSize: 17, color: T.slate }} />
                </InputAdornment>
              ),
              endAdornment: search ? (
        ...
        {loading ? (
          ...
          <Box sx={{ py: 10, textAlign: 'center' }}>
            ...
            {!search && (
              <Button
                variant="outlined" startIcon={<PersonAddRounded />} onClick={openAdd}
                sx={{
                  borderRadius: '10px', textTransform: 'none', fontWeight: 600,
                  borderColor: T.navy, color: T.navy,
                  '&:hover': { borderColor: T.gold, color: T.gold, background: `${T.gold}0A` },
                }}
              >
                Add First Client
              </Button>
                ...
                <TableRow sx={{ background: '#F4F7FA' }}>
                  {['#', 'Client', 'Firm', 'Contact', 'Location', ''].map((h, i) => (
                    <TableCell
                      ...
                      sx={{
                        fontSize: '0.68rem', fontWeight: 800, color: T.slate,
                        textTransform: 'uppercase', letterSpacing: '0.08em',
                        py: 1.4, borderBottom: `1px solid ${T.border}`,
                        whiteSpace: 'nowrap',
                      }}
                    >
                      {h}
                    ...
                    >
                      ...
                      <TableCell sx={{ py: 1.5 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                          ...
                          <Box>
                            <Typography sx={{ fontWeight: 600, fontSize: '0.85rem', color: T.navy, lineHeight: 1.2 }}>
                              {name}
                            </Typography>
                            {client.address && (
                      ...
                      <TableCell sx={{ py: 1.5 }}>
                        {fName ? (
                          <Chip
                            icon={<BusinessRounded sx={{ fontSize: '12px !important', color: '#1565c0 !important' }} />}
                            label={fName}
                            size="small"
                            sx={{
                          ...
                          <Tooltip title="Edit client" arrow>
                            <IconButton
                              ...
                              sx={{
                                ...
                                '&:hover': { background: T.navy, color: '#fff', borderColor: T.navy, transform: 'scale(1.05)' },
                          ...
                          <Tooltip title="Delete client" arrow>
                            <IconButton
                              ...
                              sx={{
                                ...
                                border: `1px solid ${T.border}`,
                                borderRadius: '8px', width: 30, height: 30,
                                '&:hover': { background: '#FEF2F2', color: T.red, borderColor: '#FECACA', transform: 'scale(1.05)' },
                                transition: 'all 0.18s',
                              }}
                            >
        ...
        {!loading && filtered.length > 0 && (
          <Box sx={{
            ...
            <Box sx={{ display: 'flex', gap: 0.5 }}>
              {[T.navy, '#1565c0', T.green].map((c, i) => (
                <Box key={i} sx={{ width: 6, height: 6, borderRadius: '50%', background: c, opacity: 0.4 }} />
              ))}
            </Box>
          </Box>
        )}
      ...
      <Dialog
        ...
        PaperProps={{
          sx: {
            borderRadius: '20px',
            boxShadow: '0 24px 80px rgba(13,27,42,0.18)',
            border: `1px solid ${T.border}`,
            overflow: 'hidden',
          },
        }}
      >
        ...
        <Box sx={{
          height: 3,
          background: `linear-gradient(90deg, transparent, ${T.gold} 30%, ${T.goldL} 65%, transparent)`,
        }} />

        <DialogTitle sx={{
          display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          px: 3, py: 2.5, background: T.cream,
          borderBottom: `1px solid ${T.border}`,
        }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <Box sx={{
          ...
          <IconButton
            size="small" onClick={closeDialog} disabled={saving}
            sx={{
              borderRadius: '8px', border: `1px solid ${T.border}`,
              '&:hover': { background: '#FEF2F2', color: T.red },
            }}
          >
        ...
        <DialogContent sx={{ px: 3, pt: 3, pb: 1, background: T.white }}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <TextField
                select fullWidth label="Associated Firm" name="firm"
              ...
              >
                <MenuItem value="">
                  <Typography color="text.secondary" variant="body2">— No Firm —</Typography>
                </MenuItem>
                {firms.length === 0 ? (
                  <MenuItem disabled>
                    <Typography variant="body2" color="text.disabled">No firms registered yet</Typography>
                  </MenuItem>
                ) : firms.map((firm) => (
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                sx={FIELD_SX}
                InputProps={{
                  startAdornment: <InputAdornment position="start"><EmailOutlined sx={{ fontSize: 16, color: T.slate }} /></InputAdornment>,
                }}
              />
            </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                sx={FIELD_SX}
                InputProps={{
                  startAdornment: <InputAdornment position="start"><PhoneOutlined sx={{ fontSize: 16, color: T.slate }} /></InputAdornment>,
                }}
              />
            </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                sx={FIELD_SX}
                InputProps={{
                  startAdornment: <InputAdornment position="start"><LocationOnOutlined sx={{ fontSize: 16, color: T.gold }} /></InputAdornment>,
                }}
              />
            </Grid>
        ...
        <DialogActions sx={{ px: 3, py: 2.5, background: T.cream, borderTop: `1px solid ${T.border}`, gap: 1 }}>
          <Button
            ...
            sx={{
              borderRadius: '10px', textTransform: 'none', fontWeight: 600,
              color: T.slate, border: `1px solid ${T.border}`,
              px: 2.5, '&:hover': { background: T.border },
            }}
          >
            Cancel
          </Button>
          <Button
            ...
            sx={{
              ...
              background: T.navy, color: T.white, px: 3,
              boxShadow: '0 4px 14px rgba(13,27,42,0.2)',
              '&:hover': { background: T.navy2, boxShadow: '0 6px 20px rgba(13,27,42,0.3)' },
              '&.Mui-disabled': { background: '#CBD5E1', color: '#fff' },
            }}
          >
            {saving ? (
              <CircularProgress size={16} color="inherit" />
            ) : dialog.mode === 'add' ? 'Add Client' : 'Save Changes'}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogContent sx={{ px: 3, py: 1.5 }}>
          <Typography sx={{ fontSize: '0.85rem', color: T.slate, lineHeight: 1.7 }}>
            ...
            <Typography component="span" sx={{ fontWeight: 700, color: T.navy }}>
              {deleteDialog.name}
            </Typography>
        ...
        <DialogActions sx={{ px: 3, py: 2.5, gap: 1 }}>
          ...
          <Button
            ...
            sx={{
              borderRadius: '10px', textTransform: 'none', fontWeight: 700,
              background: T.red, color: T.white, px: 3,
              boxShadow: '0 4px 14px rgba(220,38,38,0.25)',
              '&:hover': { background: '#B91C1C', boxShadow: '0 6px 20px rgba(220,38,38,0.35)' },
              '&.Mui-disabled': { background: '#CBD5E1', color: '#fff' },
            }}
          >
